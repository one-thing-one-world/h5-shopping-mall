...
<div>
    ...
    <div>
        ...
        <div class="submit_box ccc">
            <div class="submit  ccc" @click="submit">提交</div>
        </div>
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            // image: []
            //grtComment(id, rate, content, anonymous, _id, order_id, image)
            this.$api.grtComment(this.oneItem.cid,this.value,this.value2,true,this.oneItem._id,this.oneItem.order_id,[]).then(res => {
                console.log(res);
            }).catch(err => {
                console.log(err);
            })
            this.$router.push({name:co})
        }
