...
export default {
    ...
    methods: {
        ...
        registerAndLogin({nickname:this.nickname,password:this.password,verify:this.verify}){
            this.$api.getLogin({nickname:this.nickname,password:this.password,verify:this.verify}).then(res=>{
                console.log(res);
                if(res.code===200){
                    console.log(res);
                this.$router.push({name:'mine'})
