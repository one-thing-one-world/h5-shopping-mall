        ...
        <div class=" contents wb90  bgw h500 ">
            ...
            <div class=" m-t-20 ">
                <van-form @submit="onSubmit">
                    ...
                    <div class="df ps">
                        <div class="phoneverify">
                            <van-field class="phoneitem" v-model="phoneverify" label="验证码" placeholder="验证码" />
                        </div>
                        <div v-html="verifyCode" @click="clickGetVerify"></div>
                    </div>

                    <div class="sub j-around  df">
                        <van-button class="wb30" round block type="primary" native-type="submit" @click="login(username,password,phoneverify)" >
                            登录
                        </van-button>
                        <van-button class="wb30" round block type="warning" native-type="submit" @click="registerAndLogin(username,password,phoneverify)" >
                            注册
                        </van-button>
                    </div>
                </van-form>
...
export default {
    ...
    methods: {
        login(username,password,phoneverify){
            this.$api.getLogin(username,password,phoneverify).then(res=>{
                if(res.code===200){
                    console.log(res);
        ...
        registerAndLogin(username,password,phoneverify){
            this.$api.getLogin(username,password,verifyCode).then(res=>{
                if(res.code===200){
                    console.log(res);
                this.$router.push({name:'mine'})
                }
