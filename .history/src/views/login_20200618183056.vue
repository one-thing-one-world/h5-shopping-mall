        ...
        <div class=" contents wb90  bgw h500 ">
            ...
            <div class=" m-t-20 ">
                <van-form @submit="onSubmit">
                    ...
                    <div class="df ps">
                        <div class="phoneverify">
                            <van-field class="phoneitem" label="verify" v-model="verify" label="验证码" placeholder="验证码" />
                        </div>
                        <div v-html="verifyCode" @click="clickGetVerify"></div>
                    </div>

                    <div class="sub j-around  df">
                        <van-button class="wb30" round block type="primary" native-type="submit" @click="login(nickname,password,phoneverify)" >
                            登录
                        </van-button>
                        <van-button class="wb30" round block type="warning" native-type="submit" @click="registerAndLogin(nickname,password,phoneverify)" >
                            注册
                        </van-button>
                    </div>
                </van-form>
...
export default {
    ...
    methods: {
        login(nickname,password,verify){
            this.$api.getLogin(nickname,password,verify).then(res=>{
                if(res.code===200){
                    console.log(res);
        ...
        registerAndLogin(nickname,password,verify){
            this.$api.getLogin(nickname,password,verify).then(res=>{
                console.log(res);
                if(res.code===200){
                    console.log(res);
