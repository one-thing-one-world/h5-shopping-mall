        ...
        value(val) {
            ...
            this.$api.getSearch(val).then(res => {
                ...
                this.searchs.map((item, index) => {
                    ...
                    let replaceString =  '<span class="highlights-text">' + this.value + "</span>"// 高亮替换v-html值
                    item.names = item.names.replace(replaceReg, replaceString);
                })
                this.$emit('sends', this.searchs)
