        ...
        <div class="container " ref="container">
            <div class="contentbox" v-if="somedata.length>0">
                <van-tabs v-model="active" scrollspy >
                    ...
                    <van-tab @click="selectSomePic(item,index)" ellipsis='false' class="content" v-for="(item,index) in somedata[aIndex].bxMallSubDto" :key="index" :title="item.mallSubName">
                        
                        {{item.mallSubId}}-{{index}}
...
export default {
    ...
    methods: {
        ...
        getList(){
            let arr = this.somedata[this.aIndex].bxMallSubDto
            arr.map((item,index)=>{

            })
        },
        selectSomePic(item,index){
            console.log(index)
        }
