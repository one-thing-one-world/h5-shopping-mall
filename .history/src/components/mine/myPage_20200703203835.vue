...
export default {
    ...
    methods: {
        gosetting(){
            this.$router.push({name:'personalDatas'})
        },
      ...
      gotoSomePage(item,index){
        ...
        if(index===4){
          this.$router.push({name:'myOrder',query:{
              yy : 4
          }})
        }
        ...
        goSomePage(item,index){
            ...
            if(index===3){
                this.$router.push({name:'lookThrough'})
            } 
        },
        getInformation(){
            this.userinformation = JSON.stringify(localStorage.getItem('personList'))
        }
